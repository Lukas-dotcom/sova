// ==UserScript==
// @name         Shoptet Parameter Sorting Robot External
// @namespace    http://tampermonkey.net/
// @version      2.0.3
// @description  Externě hostovaný skript pro řazení hodnot parametrů v Shoptetu dle CSV pravidel. Tento kód se načte dynamicky skrze SOVA.
...
(function() {
    ...
    // Použijeme polling – každých 500 ms kontrolujeme, zda flag "sova:launchSPSorting" (uložený jako řetězec) je "true".
    const pollingInterval = 500; // ms
    let pollingTimer = setInterval(() => {
        let launchFlag = GM_getValue("sova:launchSPSorting", "false");
        console.log(`[SPSorting] (Polling) sova:launchSPSorting = ${launchFlag}`);
        if (launchFlag === "true") {
            clearInterval(pollingTimer);
            // Nastavíme flag na "false", aby se zabránilo opakovanému spuštění
            GM_setValue("sova:launchSPSorting", "false");
            mainSPSorting();
        }
    }, pollingInterval);
    ...
    function log(msg) {
        console.log(`[SPSorting] ${msg}`);
    }
    
    async function mainSPSorting() {
        log("Flag byl detekován, spuštění Shoptet Parameter Sorting Robot.");

        // Načteme CSV URL z flagu uloženého SOVA skriptem.
        // Pokud není nastavena, použijeme placeholder "XXXXXXXX".
                    ...
                    onload: function(response) {
                        if (response.status === 200) {
                            try {
                                ...
                                let result = {};
                                // Předpokládáme, že první řádek obsahuje hlavičky – začínáme od řádku 1.
